...
;(function($) {
  ...
  lp.setChar = function(char, charFrom, ratio) {
    ...
    if (ratio >= 1) {
      ...
      offsetFrom = this.getCharOffset(charFrom)
      $(this.lowerObject).css({
        "background-position":
          "-" +
          offsetFrom * this.settings.charWidth +
  ...
  sp.setText = function(text, textFrom) {
    ...
    if (animated) {
      ...
      for (i = 0, l = this.letters.length; i < l; i++) {
        var al = {
          ratio: 0,
          speed:
  ...
  $.fn.splitFlap = function(options) {
    ...
    return this.each(function() {
      var text = settings.text.toUpperCase()
      if (text === "") {
        text = $(this)
          .html()
          .toUpperCase()
      ...
      // Verify chars
      for (i = 0, l = textInit.length; i < l; i++) {
        c = textInit.charAt(i)
        if (settings.charsMap.indexOf(c) < 0) {
